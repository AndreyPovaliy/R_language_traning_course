...
?aggregate
#x = что, by = как разделить, FUN = какая функция
...
descr2$V
descr2$S
# x = какие векторы будут исследованы, group = группировка
# mat = вывод в виде матрицы, digits = количество знаков после запятой, fast = компактный
...
# задаем пропущенные
df$mpg[1:10]  <- NA
# пропускаем пропущенные
mean(df$mpg, na.rm = T)
# по умолчанию пропускает
aggregate(mpg ~am, df, sd)
# по умолчанию удаляет все строки с пропущенными значениями
describe(na.rm = T)
