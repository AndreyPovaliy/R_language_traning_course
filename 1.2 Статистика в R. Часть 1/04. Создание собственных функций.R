...
#функция вывода среднего или медианы, в зависимости от нормальности распределения
my_na_rm  <- function(x){
  if (is.numeric(x)){
...
######################
#dir - что находится в директории; pattern = какие фалы смотрим
...
read_data  <- function(){
  ...
  for (i in dir(pattern = "*.csv")){
    ...
    df  <- rbind(temp_df, df) # складываем файлы
